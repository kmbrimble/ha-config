...
- `npm run test:unit` — node unit tests in `tools/tests/` (the deploy lock and `www/kiosk-energy.js`); needs no HA
...
3. **Never modify the four storage-mode dashboards.** Only Kiosk and WallPanel are YAML-mode;
   the others (`lovelace`, `dash_blinds`, `dashboard_lights`, `map`) remain UI-managed and are
   off limits. (Two were deleted on 2026-08-22: `energy_cost_comparison`, removed with the energy
   comparison feature on explicit one-off approval, and `dashboard_electricity`, removed by the
   user themselves.)
...
- **Kiosk energy chart (issue #16)** is `custom:apexcharts-card` fed by `www/kiosk-energy.js`,
  ...
  point a chart at them. Deploying `www/kiosk-energy.js` means, inside `deploy_lock.py run`,

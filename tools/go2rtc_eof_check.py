#!/usr/bin/env python3
"""Detect bursts of go2rtc RTSP-producer EOF errors on the kiosk's dual-lens
"sub" camera streams (subsideyard, subbottomyard, subfront, subtopyard,
subbackgate). See kmbrimble/ha-config#22 and claude/kiosk-camera-framerate.md.
...
Reads the long-lived token from /config/tools/.ha_token (live-config only, never (already used elsewhere in
this repo, e.g. by the Playwright suite). Talks to HA's own websocket API on
127.0.0.1, same as tools/haws.py, but self-contained so it can run as a
command_line command inside the HA process itself.
"""
import base64
import json
import os
...
def main():
    ...
    total_count = 0
    for e in entries:
        if e.get("name") != "homeassistant.components.go2rtc.server":
            continue
        text = " ".join(e.get("message", [])) if isinstance(e.get("message"), list) else e.get("message", "")
        if "error=EOF" in text and any(cam in text for cam in SUB_CAMERAS):
